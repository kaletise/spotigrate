...
class Logger:
    ...
    def test(self, message):
        timestamp = datetime.datetime.now().isoformat()
        msg_nc = f'[{timestamp}] [TEST] {message}'
        if use_colors:
            msg = (f'{colorama.Fore.CYAN}[{timestamp}]'
                   f' {colorama.Back.MAGENTA}{colorama.Fore.WHITE}'
                   f'[TEST]{colorama.Style.RESET_ALL} {colorama.Fore.WHITE}'
